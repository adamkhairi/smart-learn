...
export const ContentPreview = React.memo<ContentPreviewProps>(function ContentPreview({
    content,
    type,
    maxLength = 100,
    className = ""
    ...
    return (
        <div className={`mt-2 p-2 rounded text-xs border ${styles.container} ${className}`}>
            <p className={`font-medium mb-1 ${styles.label}`}>
                {styles.labelText}
            </p>
            <p className={`line-clamp-2 ${styles.text}`}>
                {preview}
            </p>
        </div>
    );
...
export const QuestionPreview = React.memo<QuestionPreviewProps>(function QuestionPreview({
    questions
}) {
    ...
    return (
        <div className="mt-2 p-2 rounded text-xs border bg-purple-50 border-purple-200">
            <p className="font-medium mb-1 text-purple-700">
            ...
            <div className="space-y-1">
                {questionSummary.previewQuestions.map((question, index) => (
                    <p key={question.id} className="text-purple-600 line-clamp-1">
                        {index + 1}. {question.question_text.substring(0, 60)}
                        {question.question_text.length > 60 ? '...' : ''}
                    </p>
                ))}
                {questionSummary.count > 2 && (
                    <p className="text-purple-500 italic">
                        + {questionSummary.count - 2} more questions
                    </p>
                )}
            </div>
        </div>
